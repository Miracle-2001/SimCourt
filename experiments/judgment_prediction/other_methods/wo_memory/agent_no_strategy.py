...
class Agent:
    ...
    def check_hallucination(self,context:Context):
        self.strategy=self.speak("最新的庭审记录:"+context.content+"目前的证据："+str(self.evidence_pool)+"目前的策略："+str(self.strategy),
                        """
    ...
    def reflect_and_update(self,context:Context,summary=True):
        if self.role=="审判长":
            ...
            #             """
            #             )
            info="最新的庭审记录:"+context.content+"目前的证据："+str(self.evidence_pool)+"目前的辩论焦点与查明情况："+str(self.debate_focus)
            task="""
            ...
            info="最新的庭审记录:"+context.content+"目前的证据："+str(self.evidence_pool)+"拟定的目标："+self.goal+"目前的策略："+str(self.strategy)
            task="""
    ...
    def execute(
        ...
        if simple==1: #审判长宣读辩论焦点
            ...
            context="目前的辩论焦点和查明情况："+str(self.debate_focus)
            ...
            context="庭审记录："+history_context+"目前的辩论焦点和查明情况："+str(self.debate_focus)
            return self.speak(context, prompt)
        elif simple==5: # 被告人回答是否之前有过其他法律处分
            return self.speak("", prompt)
        
        if self.role=="审判长":
            context="目前的辩论焦点和查明情况："+str(self.debate_focus)+"最新的庭审记录:"+history_context
        elif self.role=="被告人":
            ...
            # context="之前法庭人员发言记录总结："+str(self.memory) if simplify==False else ""
            context="目前的策略："+str(self.strategy)+"最新的庭审记录:"+history_context
            
        if simple==3: #公诉人/辩护人 法庭调查的提问
            context+="拟询问的问题（仅供参考）："+str(self.questions)
...
class Agent_litigants(Agent):
    ...
    def preparation(self,all_term,prosecution_statement,defendant_information,evidence):
        ...
        if self.role=="公诉人":
            self.questions=self.speak("起诉状："+prosecution_statement+"被告人信息："+defendant_information+"证据："+evidence+"最终目标："+self.goal+self.strategy+"参考资料："+str(QA_pair),
                """
