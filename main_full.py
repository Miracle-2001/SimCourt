# 注意，目前是非匿名的prosecution_statement
import json
import os
import random
import logging
...
class CourtSimulation(frontEnd):
    ...
    def create_agent(self, all_description, role_config, modelname, fact=None, log_think=False):
        ...
        if role_config.get("role", None)=="审判长":
            ...
            nowAgent.preparation(self.case_data["charge"],self.case_data["prosecution_statement"],self.case_data["defendant_information"],"公诉人出示的："+str(self.case_data["evidence"][0])+"辩护人出示的："+str(self.case_data["evidence"][1]))
            return nowAgent
        else:
            ...
            if role_config.get("role", None)!="被告人":
                ...
                nowAgent.preparation(self.case_data["charge"],self.case_data["prosecution_statement"],self.case_data["defendant_information"]+request+fact,"公诉人出示的："+str(self.case_data["evidence"][0])+"辩护人出示的："+str(self.case_data["evidence"][1]))
            return nowAgent
    ...
    def run_simulation(self, defendant_info=None, prosecution_statement=None, evidence_prosecution=None, evidence_defendant=None,simulation_id=None,source: Optional[Literal["LJP", "video"]] = None):
        ...
        self.defendant=self.create_agent(self.config["all_description"], self.config["defendant"], self.defendant_model, fact=self.case_data["fact"],log_think=self.log_think)
        ...
        if request!="":
            request="被告人诉求："+request
        fact=""
        if "fact" in self.case_data:
            fact=self.case_data["fact"]
        if fact!="":
            fact="被告人认为的事实："+fact
        strategy_prompt,goal,strategy=self.advocate.for_defendant(self.case_data["charge"],self.case_data["prosecution_statement"],self.case_data["defendant_information"]+request+fact,"公诉人出示的："+str(self.case_data["evidence"][0])+"辩护人出示的："+str(self.case_data["evidence"][1]))
        # self.defendant.set_strategy(strategy_prompt,goal,strategy)
        self.defendant.set_strategy(strategy_prompt,goal,strategy,self.case_data["charge"],self.case_data["prosecution_statement"],self.case_data["defendant_information"]+request+fact,"公诉人出示的："+str(self.case_data["evidence"][0])+"辩护人出示的："+str(self.case_data["evidence"][1]))
    ...
    def save_court_log(self, file_path):
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            json.dump(self.full_history+self.thoughts+[self.panjue], f, ensure_ascii=False, indent=2)
